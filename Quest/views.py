...
def Move_request(request):
    ...
    if (location.location_type == 'Empty'):
        ...
        this_location = Location_type_empty.objects.get(location_name=location.location_name)
    ...
    if (location.location_type == 'Riddle'):
        this_location = Location_type_riddle.objects.get(location_name=location.location_name)
    ...
    if (location.location_type == 'Enemy'):
        this_location = Location_type_enemy.objects.get(location_name=location.location_name)
...
def Riddel_page(request):
    ...
    location = Location_type_riddle.objects.get(location_name=location_name)
...
def Trap_page(request):
    ...
    if randint(0, 100) < 35 and trap_location.trap_exists == True:
        ...
        if this_player_health - trap_damage < 0:
            ...
            contex = {'player_name': player_name, 'reason_name': trap_location.name, 'is_player_dead': is_player_dead,
                      'text': f'Игрок {player_name} ловушка в {trap_location.name} оказалась слишком коварна'}
        ...
        context = {'player_name': player_name, 'player_health': this_player_health,
                   'location_name': trap_location.name, 'location_description': trap_location.description, 'is_player_dead': is_player_dead,
...
def Battel_page(request):
    ...
    location_name = request.GET.get('location_name', 'Default')
    location_map_id = request.GET.get('location_map_id', '3')
    location = Location_type_enemy.objects.get(location_name=location_name)
    ...
    if battel.Player_current_health <= 0:
        text = f'Игрок {player_name} был побежден {enemy_name}'
        url_temp = f'http://127.0.0.1:8000/Gameover/?text={text}&player_name={player_name}'
        return redirect(url_temp)
    turn += 1
    inventory = make_inventory(player, battel, turn)
    context = {'player_name': player_name, 'enemy_name': enemy_name, 'location_name': location.location_name,
...
def Location_page(request):
    player_name = request.GET.get('player_name', 'Player')
    location_name = request.GET.get('location_name', 'Default')
    location = Location_type_empty.objects.get(location_name = location_name)
    location_map_id = request.GET.get('location_map_id', '3')
    context = {'player_name': player_name, 'location_name': location.location_name,
...
def Enter_dungeon_page(request):
    ...
    location = Location.objects.get(id_number=map_location.location_id)
    location_description = Location_type_empty.objects.get(location_name=location.location_name)
    player = Player.objects.get(name=player_name)
    player_class = Player_class.objects.get(class_id=player.player_class)
...
def Winning_page(request):
    ...
    location = Location_type_empty.objects.get(location_name=this_location.location_name)
    context = {'player_name': player_name, 'location_name': location.location_name,
